...
#nbdev_cell auto 0
__all__ = ['extract_comments', 'NotebookProcessor', 'ModuleMaker', 'retr_exports', 'relative_import', 'absolute_import', 'update_import', 'find_var', 'read_var', 'update_var', 'ExportModuleProcessor', 'nbdev_build_lib']
...
#nbdev_cell ../nbs/01_export.ipynb 31
#export
...
#nbdev_cell ../nbs/01_export.ipynb 37
#export
...
#nbdev_cell ../nbs/01_export.ipynb 41
#export
@patch
...
def find_var(lines, varname):
    "Find the line number that `varname` is defined in `lines`"
    return first(i for i,o in enumerate(lines) if o.startswith(varname))

def read_var(code, varname):
    "Eval and return the value of `varname` defined in `code`"
    lines = code.splitlines()
    loc = find_var(lines, varname)
    if loc is None: return None
    return eval(lines[loc].split('=')[-1].strip())

def update_var(fn, varname, func):
    ...
    res = func(v)
    loc = find_var(lines, varname)
    lines[loc] = f"{varname} = {res}"
    fn.write_text('\n'.join(lines))


#nbdev_cell ../nbs/01_export.ipynb 46
#export
@patch
def _update_all(self:ModuleMaker, all_cells, alls): return str(alls + self.make_all(all_cells))

@patch
def _make_exists(self:ModuleMaker, cells, all_cells=None):
    "`make` for `is_new=False`"
    if all_cells: update_var(self.fname, '__all__', partial(self._update_all, all_cells))
    with self.fname.open('a') as f:
        export_cells(cells, self.hdr, f)


#nbdev_cell ../nbs/01_export.ipynb 51
#export
...
#nbdev_cell ../nbs/01_export.ipynb 62
#export
@patch
...
#nbdev_cell ../nbs/01_export.ipynb 70
#export
...
#nbdev_cell ../nbs/01_export.ipynb 71
#export
@call_parse
def nbdev_build_lib(
