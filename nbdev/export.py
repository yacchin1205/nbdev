...
#nbdev_cell auto 0
__all__ = ['extract_comments', 'NotebookProcessor', 'process_comment', 'process_cell', 'no_cmd', 'process', 'ModuleMaker', 'make_all', 'relative_import', 'absolute_import', 'update_import', 'import2relative', 'import2absolute', 'make', 'ExportModuleProcessor', 'default_exp_code', 'exporti_code', 'export_code', 'create_modules', 'nbdev_build_lib']
...
@patch
def no_cmd(self:NotebookProcessor, cell):
    return cell
...
class ModuleMaker:
    ...
    def __init__(self, dest, name, nb_path, is_new=True):
        ...
        self.hdr = cell_header(self.dest2nb, Config().lib_path)
...
def make_all(self:ModuleMaker, cells):
    ...
    defs = L(o.name for o in parsed if isinstance(o,_def_types))
...
@patch
def import2absolute(cell:NbCell, libname):
    if not getattr(cell,'parsed',None): return
    src = update_import(cell.source, cell.parsed, libname, absolute_import)
    if src: cell.set_source(src)
...
def create_modules(self:ExportModuleProcessor):
    self.process()
    for (mod,cells),all_cells in zip(self.modules.items(), self.in_all.values()):
...
def nbdev_build_lib(
    ...
    if nbs is None: files = L(cfg.nbs_path.glob('*.ipynb')).filter(_use_nb)
